...
// Elastic Sprite
//
// Copyright(c) 2017-2021 M.J.Silk
//
...
void ElasticSprite::priv_updateVertices(sf::Transform transform) const
{
	...
	if (m_useShader && m_usePerspectiveInterpolation && m_pTexture != nullptr)
	{
		...
		m_vertices[2].texCoords = { m_weights[2] * ((m_actualTextureRect.left + m_actualTextureRect.width) / textureSize.x), m_weights[2] * (m_actualTextureRect.top / textureSize.y) };
		m_vertices[3].texCoords = { m_weights[3] * ((m_actualTextureRect.left + m_actualTextureRect.width) / textureSize.x), m_weights[3] * ((m_actualTextureRect.top + m_actualTextureRect.height) / textureSize.y) };
	}
	else
	{
		m_vertices[0].texCoords = { m_actualTextureRect.left, m_actualTextureRect.top };
		m_vertices[3].texCoords = { m_actualTextureRect.left + m_actualTextureRect.width, m_actualTextureRect.top + m_actualTextureRect.height };
		m_vertices[1].texCoords = { m_vertices[0].texCoords.x, m_vertices[2].texCoords.y };
		m_vertices[2].texCoords = { m_vertices[2].texCoords.x, m_vertices[0].texCoords.y };
	}
}
