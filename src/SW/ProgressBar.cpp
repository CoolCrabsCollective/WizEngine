...
void ProgressBar::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	if (m_showBar)
	{
		...
		target.draw(&m_bar.front(), 4, sf::PrimitiveType::TriangleStrip, states);
	}
}
...
void ProgressBar::priv_updateGraphics()
{
	...
	m_bar[2].position = { 0.f, m_size.y };
	m_bar[3].position = { width, m_size.y };
	...
	m_bar[2].texCoords = { textureRect.left, textureRect.top + textureRect.height };
	m_bar[3].texCoords = { textureRect.left + textureRect.width, textureRect.top + textureRect.height };
