...
// BitmapText
//
// Copyright(c) 2014-2021 M.J.Silk
//
...
{
	this->Transformable::setScale(sf::Vector2f(static_cast<float>(scaleX), static_cast<float>(scaleY)));
}
...
void BitmapText::priv_updateVertices()
{
	...
	{
		m_vertices.clear();
		m_bounds = sf::FloatRect(sf::Vector2f(0.f, 0.f), sf::Vector2f(0.f, 0.f));
	}

	m_vertices.resize(m_string.length() * 6);
	...
	for (unsigned int character{ 0 }; character < m_string.length(); ++character)
	{
		...
		m_vertices[character * 6 + 0].position = glyphPosition;
		m_vertices[character * 6 + 1].position = glyphPosition + sf::Vector2f(static_cast<float>(glyph.textureRect.width), 0);
		m_vertices[character * 6 + 2].position = glyphPosition + sf::Vector2f(0, static_cast<float>(glyph.textureRect.height));

		m_vertices[character * 6 + 3].position = glyphPosition + sf::Vector2f(static_cast<float>(glyph.textureRect.width), 0);
		m_vertices[character * 6 + 4].position = glyphPosition + sf::Vector2f(0, static_cast<float>(glyph.textureRect.height));
		m_vertices[character * 6 + 5].position = glyphPosition + sf::Vector2f(static_cast<float>(glyph.textureRect.width), static_cast<float>(glyph.textureRect.height));

		m_vertices[character * 6 + 0].texCoords = sf::Vector2f(
			static_cast<float>(glyph.textureRect.left),
			static_cast<float>(glyph.textureRect.top));
		m_vertices[character * 6 + 1].texCoords = sf::Vector2f(
			static_cast<float>(glyph.textureRect.left + glyph.textureRect.width),
			static_cast<float>(glyph.textureRect.top));
		m_vertices[character * 6 + 2].texCoords = sf::Vector2f(
				static_cast<float>(glyph.textureRect.left),
				static_cast<float>(glyph.textureRect.top + glyph.textureRect.height));
		m_vertices[character * 6 + 3].texCoords = sf::Vector2f(
				static_cast<float>(glyph.textureRect.left + glyph.textureRect.width),
				static_cast<float>(glyph.textureRect.top));
		m_vertices[character * 6 + 4].texCoords = sf::Vector2f(
				static_cast<float>(glyph.textureRect.left),
				static_cast<float>(glyph.textureRect.top + glyph.textureRect.height));
		m_vertices[character * 6 + 5].texCoords = sf::Vector2f(
				static_cast<float>(glyph.textureRect.left + glyph.textureRect.width),
				static_cast<float>(glyph.textureRect.top + glyph.textureRect.height));
		...
		minX = std::min(minX, m_vertices[character * 6 + 0].position.x);
		maxX = std::max(maxX, m_vertices[character * 6 + 5].position.x);
		minY = std::min(minY, m_vertices[character * 6 + 0].position.y);
		maxY = std::max(maxY, m_vertices[character * 6 + 5].position.y);
	}
